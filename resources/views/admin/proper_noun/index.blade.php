...
@section('pageTitle', __('admin.header.固有名詞').__('admin.一覧'))
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                        ...
                        <div class="panel panel-default">
                            <div class="panel-body">
                                @if(auth()->user()->can('proper_noun create'))
                                    <a class="btn btn-block btn-default" href="{{ route('admin.proper_noun.create') }}">{{__('admin.新規追加')}}</a>
                                @else
                                    <a class="btn btn-block btn-default" disabled="">{{__('admin.新規追加')}}</a>
                                @endif
                                @if(auth()->user()->can('proper_noun import'))
                                    <a class="btn btn-block btn-default" href="{{ route('admin.proper_noun.import') }}">{{__('admin.インポート')}}</a>
                                @else
                                    <a class="btn btn-block btn-default" disabled="">{{__('admin.インポート')}}</a>
                                @endif
                                @if(auth()->user()->can('proper_noun export'))
                                    <a class="btn btn-block btn-default" href="{{ route('admin.proper_noun.export') }}">{{__('admin.エクスポート')}}</a>
                                @else
                                ...
                                <table class="table data-table" id="dtable_proper_noun" style="width: 100%;" data-tables='@json([
                                   ...
                                   'form' => ['search_form']
                                    ...
                                    <tr>
                                        ...
                                        <th data-name="word">{{ __('admin.proper_noun.固有名詞') }}</th>
                                        <th data-template="true">
                                            <template>
                                                @if(auth()->user()->can('proper_noun edit'))
                                                    <a class="btn btn-default" href="{{ route('admin.proper_noun.edit',['proper_noun'=>'%id%']) }}">{{__('admin.修正')}}</a>
                                                @else
